...
timer: 5000
});setTimeout(function(){ location.href = "/mua-page" },5000);
</script>
    <?php 
    } else if($sl < 20) { ?>
<script type="text/javascript">
cuteToast({
type: "error",
message: "Mua Tối Thiểu 20 Like Page Trở Lên !",
timer: 5000
});setTimeout(function(){ location.href = "/mua-page" },5000);
</script>   
<?php } else {
...
"referer:https://traodoisub.com/mua/fanpage/",
);
$data = 'maghinho=&id='.$id.'&sl='.$sl.'&is_album=not&speed=1&dateTime='.$tong.'';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/fanpage/themid.php');
...
$create = $ketnoi->query("INSERT INTO `mualikepage` SET 
        `idtang` = '$id',
        `sl` = '$sl',
        `username` = '$username',
        `time` = '$time' ");
...
?>
                                <form action="" method="POST">
                                    <div class="form-group row">
                                        <label for="" class="col-md-3">ID Page </label>
                                        <div class="col-md-9">
                                            <input type="number" class="form-control mb-3" name="id" placeholder="Nhập id page cần tăng">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                                ...
                                                <div class="form-check">
                                                    ...
                                                    <label class="form-check-label" for="sv1">sv1 <small>(Like page người dùng chéo, tài khoản tên Việt, có Avatar)</small>&nbsp;<span class="badge border border-success text-success">10 Coin / 1 like</span>&nbsp;<b class="text-warning">(Hoạt động)</b></label>
                                                </div>
                                            </div>
                                                                                    </div>
                                    </div>
...
$data = 'page=1&query=';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/fanpage/fetch.php');
curl_setopt($mr, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($mr, CURLOPT_COOKIEFILE, 'file.txt');
curl_setopt($mr, CURLOPT_POSTFIELDS, $data);
curl_setopt($mr, CURLOPT_HTTPHEADER, $header);
...
<?php
                                $i = 1;
                                $query = mysqli_query($ketnoi, "SELECT * FROM `mualikepage` WHERE `username` = '$username' ORDER BY id DESC limit 5");

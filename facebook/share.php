...
timer: 5000
});setTimeout(function(){ location.href = "/mua-share" },5000);
</script>
    <?php 
    } else if($sl < 20) { ?>
<script type="text/javascript">
cuteToast({
type: "error",
message: "Mua Tối Thiểu 20 Share Trở Lên !",
timer: 5000
});setTimeout(function(){ location.href = "/mua-share" },5000);
</script>   
<?php } else {
...
"referer:https://traodoisub.com/mua/share/",
);
$data = 'maghinho=&id='.$id.'&sl='.$sl.'&is_album=not&speed=1&dateTime='.$tong.'';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/share/themid.php');
...
$create = $ketnoi->query("INSERT INTO `muashare` SET 
        `idtang` = '$id',
        `sl` = '$sl',
        `username` = '$username',
        `time` = '$time' ");
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                                ...
                                                <div class="form-check">
                                                    ...
                                                    <label class="form-check-label" for="sv1">sv1 <small>(Share người dùng chéo, tài khoản tên Việt, có Avatar)</small>&nbsp;<span class="badge border border-success text-success">5 coin / 1 share</span>&nbsp;<b class="text-warning">(Hoạt động)</b></label>
                                                </div>
                                            </div>
                                                                                    </div>
                                    </div>
...
$data = 'page=1&query=';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/share/fetch.php');
curl_setopt($mr, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($mr, CURLOPT_COOKIEFILE, 'file.txt');
curl_setopt($mr, CURLOPT_POSTFIELDS, $data);
curl_setopt($mr, CURLOPT_HTTPHEADER, $header);
...
<?php
                                $i = 1;
                                $query = mysqli_query($ketnoi, "SELECT * FROM `muashare` WHERE `username` = '$username' ORDER BY id DESC limit 5");

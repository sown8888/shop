...
timer: 5000
});setTimeout(function(){ location.href = "/mua-youtube-cmt" },5000);
</script>
    <?php 
    } else if($sl < 50) { ?>
<script type="text/javascript">
cuteToast({
type: "error",
message: "Mua Tối Thiểu 50 Comment Trở Lên !",
timer: 5000
});setTimeout(function(){ location.href = "/mua-youtube-cmt" },5000);
</script>   
<?php } else {
...
"referer:https://traodoisub.com/mua/youtube_comment/",
);
$data = 'maghinho=&id='.$id.'&noidung=%5B%22'.$nd.'%22%5D&sl='.$sl.'&is_album=not&speed=1&dateTime='.$tong.'';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/youtube_comment/themid.php');
...
$create = $ketnoi->query("INSERT INTO `muayoutubecmt` SET 
        `idtang` = '$id',
        `sl` = '$sl',
        `nd` = '$nd',
        `username` = '$username',
...
?>
                                <form action="" method="POST">
                                    <div class="form-group row">
                                        <label for="" class="col-md-3"> Link Video: </label>
                                        <div class="col-md-9">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                                ...
                                                <div class="form-check">
                                                    ...
                                                    <label class="form-check-label" for="sv1">sv1 <small>(Comment người dùng chéo, tài khoản tên Việt, có Avatar)</small>&nbsp;<span class="badge border border-success text-success">17 coin / 1 comment</span>&nbsp;<b class="text-warning">(Hoạt động)</b></label>
                                                </div>
                                            </div>
                                                                                    </div>
                                    </div>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <input type="nd" id="nd" class="form-control mb-3" name="nd" placeholder="Nội Dung" onkeyup="tinhtien();" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
...
$data = 'page=1&query=';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/youtube_comment/fetch.php');
curl_setopt($mr, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($mr, CURLOPT_COOKIEFILE, 'file.txt');
curl_setopt($mr, CURLOPT_POSTFIELDS, $data);
curl_setopt($mr, CURLOPT_HTTPHEADER, $header);
...
$nd = $json["data"][0]["noidung"];
$time = $json["data"][0]["date"];
?>
<tbody class="list" id="table-purchase-body"><tr class="btn-reveal-trigger"> 
<?php
                                $i = 1;
                                $query = mysqli_query($ketnoi, "SELECT * FROM `muayoutubecmt` WHERE `username` = '$username' ORDER BY id DESC limit 5");

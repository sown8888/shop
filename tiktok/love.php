...
timer: 5000
});setTimeout(function(){ location.href = "/mua-tiktok_like" },5000);
</script>
    <?php 
    } else if($sl < 20) { ?>
<script type="text/javascript">
...
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/tiktok_like/themid.php');
...
$create = $ketnoi->query("INSERT INTO `muatiktoklike` SET 
        `idtang` = '$id',
        `sl` = '$sl',
        `username` = '$username',
        `time` = '$time' ");
...
?>
                                <form action="" method="POST">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <input type="number" class="form-control mb-3" name="id" placeholder="https://www.tiktok.com/@quynhh.vn/video/7041885492069125377">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                                ...
                                                <div class="form-check">
                                                    ...
                                                    <label class="form-check-label" for="sv1">sv1 <small>(Love người dùng chéo, tài khoản tên Việt, có Avatar)</small>&nbsp;<span class="badge border border-success text-success">10 coin / 1 love</span>&nbsp;<b class="text-warning">(Hoạt động)</b></label>
                                                </div>
                                            </div>
                                                                                    </div>
                                    </div>
...
$data = 'page=1&query=';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/tiktok_like/fetch.php');
curl_setopt($mr, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($mr, CURLOPT_COOKIEFILE, 'file.txt');
curl_setopt($mr, CURLOPT_POSTFIELDS, $data);
curl_setopt($mr, CURLOPT_HTTPHEADER, $header);
...
<?php
                                $i = 1;
                                $query = mysqli_query($ketnoi, "SELECT * FROM `muatiktoklike` WHERE `username` = '$username' ORDER BY id DESC limit 5");

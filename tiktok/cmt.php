...
timer: 5000
});setTimeout(function(){ location.href = "/mua-tiktok_comment" },5000);
</script>
    <?php 
    } else if($sl < 50) { ?>
<script type="text/javascript">
...
"referer:https://traodoisub.com/mua/tiktok_comment/",
);
$data = 'maghinho=&id='.$id.'&noidung=%5B%22'.$nd.'%22%5D&sl='.$sl.'&is_album=not&speed=1&dateTime='.$tong.'';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/tiktok_comment/themid.php');
...
$create = $ketnoi->query("INSERT INTO `muatiktokcmt` SET 
        `idtang` = '$id',
        `sl` = '$sl',
        `nd` = '$nd',
        `username` = '$username',
...
?>
                                <form action="" method="POST">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-9">
                                            <input type="number" class="form-control mb-3" name="id" placeholder="https://www.tiktok.com/@tungduongtool">
                                        </div>
                                    </div>
                                    <div class="form-group row">
...
$data = 'page=1&query=';
$mr = curl_init();
curl_setopt($mr, CURLOPT_URL, 'https://traodoisub.com/mua/tiktok_comment/fetch.php');
curl_setopt($mr, CURLOPT_RETURNTRANSFER, TRUE);
curl_setopt($mr, CURLOPT_COOKIEFILE, 'file.txt');
curl_setopt($mr, CURLOPT_POSTFIELDS, $data);
curl_setopt($mr, CURLOPT_HTTPHEADER, $header);
...
<?php
                                $i = 1;
                                $query = mysqli_query($ketnoi, "SELECT * FROM `muatiktokcmt` WHERE `username` = '$username' ORDER BY id DESC limit 5");
